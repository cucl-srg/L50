...
def getrtts(exp,num):
    ...
    for i in range(10):
        with open('/root/L50Lab1/'+exp+'_'+str(i)) as f:
            for j in range(num):
                rt = f.next()
...
def getrtt(ffile,num):
    rtt= []
    with open('/root/L50Lab1/'+ffile) as f:
            for j in range(num):
                rt = f.next()
                rt = findall(r'time=(.*?) ms',rt)[0]
...
def graph1(exp,div,num):
    rtts= getrtts('1/'+exp,num)
    avgs = []
    for i in range(num):
...
def graph5_001(exp,usecs,num):
    maxs_001 = [[] for u in usecs]
    mins_001 = [[] for u in usecs]
    meds_001 = [[] for u in usecs]
    for u in range(len(usecs)):
        for i in range(10):
            rtt = getrtt('5/'+exp+'_0.001_'+str(usecs[u])+'_'+str(i), num)
...
def data_iperf(fexp):
    ...
    for j in range(5):
        with open('/root/L50Lab1/'+fexp+'_' + str(j)) as f:
            for i in range(10):
                bw = f.next()
...
def data10():
    ...
    for j in range(5):
        with open('/root/L50Lab1/10/exp10_' + str(j)) as f:
            f.next()
            f.next()
            for i in range(10):
                sb = f.next()
...
def data11(windows):
    ...
    for b in range(len(windows)):
        for i in range(5):
            with open('/root/L50Lab1/11/exp11_'+str(windows[b])+'_'+str(i)) as f:
                wind = f.next()
                if (i==0):
...
def data_band(fexp,bands):
    pcs = [[] for b in bands]
    bans = [[] for b in bands]
    for b in range(len(bands)):
        for i in range(5):
            with open('/root/L50Lab1/'+fexp+'_'+str(bands[b])+'_'+str(i)) as f:
                out = f.read()
...
def data13b(windows):
    ...
    for b in range(len(windows)):
        for i in range(5):
            with open('/root/L50Lab1/13/exp13b_'+str(windows[b])+'_'+str(i)) as f:
                for i in range(3):
                    f.next()
                bb = f.next()
